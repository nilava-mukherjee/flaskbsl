...
def currentdata():
    ...
    try:
        db2 = mysql.connector.connect(user="ajarcake4", password="xJkuyOKBizuim9M42mukRA", host="server050641860.mysql.database.azure.com", database="bokaro_ems", port="3306")
        db2_cursor = db2.cursor()
        db2_cursor.execute("SELECT Modbus_time,AVG_voltage_LL,AVG_current,THDP1,Total_kVA,AVG_pf,Total_net_kWh,Total_kW,Frequency,pf1 from trialbsl WHERE Meter_id =%s",(Meter_id,))
        data7 = db2_cursor.fetchall()
        db2.commit()
        db2.close()
        print(data7)
        r_values = pd.DataFrame(data7, columns=['Modbus time', 'Average Voltage LL', 'Average Current', 'Total harmonic distortion', 'Apparent Power',
                                                'Average power factor', 'Total energy', 'Total power',
                                                'Frequency', 'pf1'])
...
def getPlotCSV():
    ...
    try:
        ...
        db2.commit()
        db2.close()
        m_values = pd.DataFrame(data18, columns=['Modbus time', 'Average Voltage LL', 'Average Current', 'Total harmonic distortion', 'Apparent Power',
                                                'Average power factor', 'Total energy', 'Total power',
                                                'Frequency', 'pf1'])
        ...
        csv = m_values.to_string(columns=['Modbus time', 'Average Voltage LL', 'Average Current', 'Total harmonic distortion', 'Apparent Power',
                                                'Average power factor', 'Total energy', 'Total power',
                                                'Frequency', 'pf1'], header = 'false',index = 'false' )
...
def getexcel():
    ...
    try:
        ...
        db2.commit()
        db2.close()
        t_values = pd.DataFrame(data13, columns=['Modbus time', 'Average Voltage LL', 'Average Current', 'Total harmonic distortion', 'Apparent Power',
                                                'Average power factor', 'Total energy', 'Total power',
                                                'Frequency', 'pf1'])
...
def historical():
    ...
    try:
        ...
        db2.commit()
        db2.close()
        h_values = pd.DataFrame(data8, columns=['Modbus time', 'Average Voltage LL', 'Average Current', 'Total harmonic distortion', 'Apparent Power',
                                                'Average power factor', 'Total energy', 'Total power',
                                                'Frequency', 'pf1'])
...
def historicdwnld():
    ...
    try:
        ...
        db2.commit()
        db2.close()
        t1_values = pd.DataFrame(data5, columns=['Modbus time', 'Average Voltage LL', 'Average Current', 'Total harmonic distortion', 'Apparent Power',
                                                'Average power factor', 'Total energy', 'Total power',
                                                'Frequency', 'pf1'])
        ...
        csv = t1_values.to_string(columns=['Modbus time', 'Average Voltage LL', 'Average Current', 'Total harmonic distortion', 'Apparent Power',
                                                'Average power factor', 'Total energy', 'Total power',
                                                'Frequency', 'pf1'], header = 'false',index = 'false' )
...
def current_data():
    ...
    try:
        ...
        db2.commit()
        db2.close()
        t2_values = pd.DataFrame(data7, columns=['Modbus Time', 'AVG_voltage_LL', 'Average current',
                                                'total_harmonic_distortion', 'Apparent Energy','Average Power Factor','Total Energy' 'Total Power',
                                                'frequency', 'pf1'])
